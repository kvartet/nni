...
class ParameterRange():
    def __init__(self, name, algorithm_name, is_categorical, size,
                 categorical_values, low, high, is_log_distributed, is_integer, aware):
        self.name = name
        self.tag = name if aware else algorithm_name + "|" + name
    ...
    @staticmethod
    def categorical(algorithm_name, name, values, aware):
        return ParameterRange(name, algorithm_name, True,
                              len(values), values, np.nan, np.nan, False, False, aware)

    @staticmethod
    def numerical(algorithm_name, name, low, high, aware,
                  is_log_distributed=False, is_integer=False):
        return ParameterRange(name, algorithm_name, False,
                              -1, None, low, high, is_log_distributed, is_integer, aware)


class SearchSpace():
    def __init__(self, json_string, aware):
        self.is_nested = True if 'branch' in json_string.keys() else False
                    ...
                    for param_kv in list(algo_kv.items())[1:]:
                        ...
                        if param_type == "choice":
                            ...
                            algo.append(ParameterRange.categorical(
                                algo_kv["_name"], param_name, values, aware))
                        else:
                            ...
                            alog.append(ParameterRange.numerical(
                                algo_kv["_name"], param_name, low, high, log, integer, aware))
        else:
            for name, value in json_string.items():
                if value["_type"] == "choice":
                    values = list()
                    for val in value["_value"]:
                        values.append(val)
                    algo = (ParameterRange.categorical(name, name, values, aware))
                    self.algorithms[name] = algo
                else:
                    print('Search space is error!')
...
class TPETuner(Tuner):
    ...
    def update_search_space(self, search_space):
        ...
        """
        self.space = SearchSpace(search_space, self.aware)
    ...
    def suggest(self, space, history):
                ...
                if algo[0] in chosen_pipeline:
                    ...
                    for param_range in algo[1]:
                        if param_range.is_categorical:
                            ...
                            # print('param_range.size:{}, index:{}'.format(param_range.size, index))
        ...
        # print('formatted_param:{}, param:{}'.format(formatted_param, param))
            ...
            for algo in self.space.algorithms.items():
                if algo[1].is_categorical:
                    index = self.suggest_categorical(history, algo[1].tag, algo[1].size)
                    param[algo[1].tag] = index
                    formatted_param[algo[1].name] = algo[1].categorical_values[index]
    ...
    def suggest_categorical(self, history, tag, size):
        ...
        if len(obs_above) == 0 or len(obs_below) == 0:
            return self.rng.integer(size)

        weights = linear_forgetting_weights(len(obs_below), self.lf)
        counts = np.bincount(obs_below, weights, size)
        p = ((counts+self.prior_weight)/np.sum(counts+self.prior_weight))[:size]
        # print('size:{}'.format(size))
        # print('p_above:{}'.format(p))
        sample = self.rng.categorical(p, self.n_ei_candidates)
        # print('sample_below:{}'.format(sample))
        below_llik = np.log([p[i] for i in sample])

        weights = linear_forgetting_weights(len(obs_above), self.lf)
        counts = np.bincount(obs_above, weights, size)
        p = ((counts+self.prior_weight)/np.sum(counts+self.prior_weight))[:size]
        # sample = self.rng.categorical(p, self.n_ei_candidates)
        # print('p:{}'.format(p))
        ...
        return self.find_best(sample, below_llik, above_llik, size)
    ...
    def ap_split_trials(self, history, tag):
        ...
        #     print('item.param.keys():{}'.format(item.param.keys()))
    ...
    def find_best(self, samples, below_llik, above_llik, size):
        llik = below_llik - above_llik
        best = np.argmax(llik)
        while samples[best] >= size:
            # print('best:{}, samples[best]:{}'.format(best, samples[best]))
            llik[best] = - np.inf
            best = np.argmax(llik)
        return samples[best]