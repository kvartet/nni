...
#     ret = [0] * max(np.max(x), len(x), min_length)
#     print('x:{}, weights:{}, min_length:{}'.format(x, weights, min_length))
#     for i in range(min(len(x), min_length, len(weights))):
#         print('i:{}, x[i]:{}'.format(i,x[i]))
#         ret[x[i]] += weights[i]
#     return np.asarray(ret[:min_length])

# def bin_count(x, weights=None, minlength=None, p=None):
